...
do.sim = function(rep, temperatures, S = 20, q = 1.2){
  ...
  run = function(temperature, fw, BM, rep, q = 1.2, S = 20){
    ...
    res = rbind.data.frame(c(temperature, S, q, rep, 0, extinct), 
                           c(temperature, S, q, rep, 1, extinct2))
    # results[c(i, i+1), ] = res
    names(res) = c('temperature', 'S', 'q', 'rep', 'prefs', 'extinctions')
    return(res)
  ...
  while (isolated & no.herbivore){
    ...
    # trophic.levels = TL(FW)
    # herbivore species are species with a trophic level of 2
    # herbivores = trophic.levels == 2
    # remove FW with no pure herbivores
    # no.herbivore = sum(herbivores) == 0
    comp = comp + 1
  ...
  xx = mclapply(temperatures, run, fw = fw,  BM = BM, rep = rep, mc.cores  = 4)
...
temperatures = seq(2, 30, by = 2)
results = mclapply(1:18, do.sim, temperatures = temperatures, q = 1.5, mc.cores = 6)
results2= do.call(rbind.data.frame, results)
results2$persistence = 1 - results2$extinctions/50


# library(reshape2)

# res_df = melt(results, id = "time")

results3 = subset(results2, temperature < 22)

g = ggplot(results3, aes(x = temperature, y = persistence, color = as.factor(prefs)))+
...
n = nrow(results2)
difs = data.frame(pers.dif = results2[(1:n)%%2 == 1,'persistence'] - results2[(1:n)%%2 == 0,'persistence'],
                  temperature = results2[(1:n)%%2 == 1,'temperature'])

g2 = ggplot(difs, aes(x = temperature, y = pers.dif))+
  # geom_point(position = 'jitter')+
  geom_point()+
  geom_smooth()
g2
